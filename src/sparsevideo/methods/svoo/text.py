...
def pad_text_clusters(dynamic_map, q_sizes, k_sizes, q_sorted_indices, text_len: int, prompt_length=None):
    ...
    if unprompt_len > 0:
        ...
        full_map = torch.zeros(B, H, nqc + 1, nkc + 1, dtype=torch.bool, device=device)
        full_map[:, :, :nqc, :nkc] = dynamic_map
        full_map[:, :, :nqc, nkc] = True
        full_map[:, :, nqc, :] = True

        text_q = torch.full((B, H, 1), int(text_len), dtype=q_sizes.dtype, device=q_sizes.device)
        text_k = torch.full((B, H, 1), int(text_len), dtype=k_sizes.dtype, device=k_sizes.device)
        q_sizes = torch.cat([q_sizes, text_q], dim=-1)
        k_sizes = torch.cat([k_sizes, text_k], dim=-1)
