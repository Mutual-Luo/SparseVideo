...
if readme_path.exists():
    ...
    long_description = "SparseVideo-owned copy of MIT Han Lab Block-Sparse-Attention for Draft Attention."
...
FORCE_BUILD = (
    os.getenv("BLOCK_SPARSE_ATTN_FORCE_BUILD", os.getenv("FLASH_ATTENTION_FORCE_BUILD", "FALSE")) == "TRUE"
)
SKIP_CUDA_BUILD = (
    os.getenv("BLOCK_SPARSE_ATTN_SKIP_CUDA_BUILD", os.getenv("FLASH_ATTENTION_SKIP_CUDA_BUILD", "FALSE")) == "TRUE"
)
# For CI, we want the option to build with C++11 ABI since the nvcr images use C++11 ABI
FORCE_CXX11_ABI = (
    os.getenv("BLOCK_SPARSE_ATTN_FORCE_CXX11_ABI", os.getenv("FLASH_ATTENTION_FORCE_CXX11_ABI", "FALSE")) == "TRUE"
)
...
BUILD_MODE = os.getenv("BLOCK_SPARSE_ATTN_BUILD_MODE", "full").lower()
if BUILD_MODE not in {"draft_inference", "full"}:
    raise RuntimeError(
        "BLOCK_SPARSE_ATTN_BUILD_MODE must be either 'draft_inference' or 'full'."
    )
...
draft_inference_sources = [
    "csrc/block_sparse_attn/flash_api.cpp",
    "csrc/block_sparse_attn/src/flash_fwd_block_hdim128_fp16_sm80.cu",
    "csrc/block_sparse_attn/src/flash_fwd_block_hdim128_bf16_sm80.cu",
]

block_sparse_attn_sources = (
    draft_inference_sources if BUILD_MODE == "draft_inference" else full_sources
)
...
if not SKIP_CUDA_BUILD:
    ...
    ext_modules.append(
        CUDAExtension(
            ...
            sources=block_sparse_attn_sources,
            extra_compile_args={
                "cxx": ["-O3", "-std=c++17"] + generator_flag + draft_inference_flags,
