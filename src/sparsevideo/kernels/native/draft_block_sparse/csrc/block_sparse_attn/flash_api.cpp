...
#ifdef SPARSEVIDEO_DRAFT_INFERENCE_ONLY
    TORCH_CHECK(false, "SparseVideo Draft inference build only includes fwd_block; rebuild with BLOCK_SPARSE_ATTN_BUILD_MODE=full for dense forward.");
#else
...
#ifdef SPARSEVIDEO_DRAFT_INFERENCE_ONLY
    TORCH_CHECK(params.d == 128, "SparseVideo Draft inference build only includes fwd_block head_dim=128; rebuild with BLOCK_SPARSE_ATTN_BUILD_MODE=full for other head dims.");
    TORCH_CHECK(params.num_splits <= 1 && !force_split_kernel, "SparseVideo Draft inference build only includes non-split fwd_block.");
    FP16_SWITCH(!params.is_bf16, [&] {
        run_mha_fwd_block_<elem_type, 128>(params, stream);
    });
#else
    FP16_SWITCH(!params.is_bf16, [&] {
...
#ifdef SPARSEVIDEO_DRAFT_INFERENCE_ONLY
    TORCH_CHECK(false, "SparseVideo Draft inference build only includes fwd_block; rebuild with BLOCK_SPARSE_ATTN_BUILD_MODE=full for backward.");
#else
    FP16_SWITCH(!params.is_bf16, [&] {
        if (params.d <= 32) {
...
#ifdef SPARSEVIDEO_DRAFT_INFERENCE_ONLY
    TORCH_CHECK(false, "SparseVideo Draft inference build only includes fwd_block; rebuild with BLOCK_SPARSE_ATTN_BUILD_MODE=full for block backward.");
#else
    FP16_SWITCH(!params.is_bf16, [&] {
