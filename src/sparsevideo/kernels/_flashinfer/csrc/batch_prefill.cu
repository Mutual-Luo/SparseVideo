...
at::Tensor BatchPrefillWithKVCachePlan(
    ...
    int64_t head_dim_vo, bool causal, int64_t window_left = -1, int64_t fixed_split_size = -1,
    bool disable_split_kv = false, int64_t num_colocated_ctas = 0) {
  ...
  cudaError_t status = PrefillPlan<IdType>(
      ...
      head_dim_qk, head_dim_vo, page_size, enable_cuda_graph, /*sizeof_dtype_o=*/2,
      (int32_t)window_left, (int32_t)fixed_split_size, disable_split_kv, num_colocated_ctas,
      stream);
